...
export default function Planner() {
  ...
  // NUEVOS ESTADOS PARA LA VENTANA EMERGENTE (MODAL)
  ...
  const openTimeModal = (task: Task) => {
    ...
    // Cargamos los horarios actuales en la ventana (si los tiene)
  ...
  const saveTimeConfig = async () => {
    ...
    // Agregamos los segundos para la base de datos o pasamos null si lo dejaron vacío
    ...
    closeTimeModal(); // Cerramos la ventana emergente al instante
  ...
  const clearTimeConfig = async () => {
    setTempStartTime("");
    setTempEndTime("");
    // Se guarda y borra tocando el botón de Guardar luego.
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto mt-8 grid grid-cols-1 md:grid-cols-3 gap-8 items-start relative">
      
      {/* CALENDARIO */}
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 shadow-sm col-span-1">
        <div className="flex justify-between items-center mb-6">
          <button onClick={prevMonth} className="p-2 hover:bg-slate-200 rounded-full transition text-slate-600">
            <ChevronLeft size={20} />
          </button>
          <h2 className="text-lg font-bold text-slate-800 capitalize">
            {format(currentDate, "MMMM yyyy", { locale: es })}
          </h2>
          <button onClick={nextMonth} className="p-2 hover:bg-slate-200 rounded-full transition text-slate-600">
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isSelected ? "bg-slate-800 text-white shadow-md hover:bg-slate-700" : "hover:bg-slate-200"}
                  ${isToday && !isSelected ? "border-2 border-slate-800 text-slate-800" : ""}
                `}
              >
                {format(day, "d")}
              </button>
      ...
      {/* DETALLE DEL DÍA */}
      <div className="col-span-1 md:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm min-h-[400px]">
        <h3 className="text-xl font-bold text-slate-800 mb-1 capitalize">
          {format(selectedDate, "EEEE, d 'de' MMMM", { locale: es })}
        </h3>
        <p className="text-slate-500 text-sm mb-6">Planificación del día</p>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex gap-2">
              <input
                ...
                className="flex-grow p-3 border border-slate-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-slate-800 text-sm disabled:bg-slate-50 text-slate-900 bg-white"
              />
              <button
                type="submit"
                disabled={isPlanning || !planText.trim()}
                className="px-6 bg-slate-800 text-white text-sm font-semibold rounded-xl shadow-sm hover:bg-slate-700 disabled:bg-slate-400 transition min-w-[120px] flex justify-center items-center"
              >
                {isPlanning ? "Pensando..." : "Planificar"}
              </button>
            </div>
            {feedbackMsg.text && (
              <span className={`text-xs font-medium px-2 ${feedbackMsg.type === 'error' ? 'text-red-500' : 'text-emerald-600'}`}>
                {feedbackMsg.text}
              </span>
            )}
          </div>
        ...
        {tasks.length > 0 && (
          <div className="mb-4 flex justify-between items-center border-b border-slate-100 pb-3">
            <button
              ...
              className="flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-slate-800 transition bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-rose-50 border border-rose-200 hover:bg-rose-100 text-rose-700 text-xs font-bold rounded-xl shadow-sm transition disabled:bg-slate-50 disabled:border-slate-100 disabled:text-slate-300"
            >
              <Trash2 size={14} />
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 text-center">
            ...
            <p className="text-slate-500 font-medium">No hay actividades planificadas para este día.</p>
          </div>
        ) : (
          /* GRID DE TAREAS */
              ...
              >
                <div className="flex items-center gap-4 flex-grow">
                  ...
                  <div className="flex flex-col gap-1 max-w-[65%]">
                    <span className={`text-base font-medium transition-all ${task.estado === 'completado' ? 'text-slate-500 line-through' : 'text-slate-800'}`}>
                      {task.titulo}
                    </span>
                  </div>
                </div>

                {/* NUEVO BOTÓN INTERACTIVO PARA ABRIR EL MODAL */}
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    openTimeModal(task);
                  ...
                  className={`flex items-center gap-1.5 border px-3 py-1.5 rounded-xl text-xs transition-colors
                    ${task.hora_inicio || task.hora_fin 
                      ? 'bg-slate-800 border-slate-800 text-white hover:bg-slate-700 shadow-sm' 
                      : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100 hover:text-slate-800'}
                  `}
                >
                  <Clock size={14} className={task.hora_inicio || task.hora_fin ? 'text-slate-200' : 'text-slate-400'} />
      ...
      {timePickerTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div 
            className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform transition-all border border-slate-100"
            onClick={(e) => e.stopPropagation()} // Evita que se cierre si hacés click adentro
          >
            <h4 className="text-xl font-bold text-slate-800 mb-1">Configurar horario</h4>
            <p className="text-sm text-slate-500 mb-6 truncate">{timePickerTask.titulo}</p>

            <div className="flex items-center justify-between gap-4 mb-8">
              <div className="flex flex-col flex-1">
                <label className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Hora Inicio</label>
                <input
                  type="time"
                  value={tempStartTime}
                  onChange={(e) => setTempStartTime(e.target.value)}
                  className="w-full p-3 border border-slate-200 rounded-xl bg-slate-50 text-slate-800 font-semibold focus:outline-none focus:ring-2 focus:ring-slate-800 text-center text-lg"
                />
              </div>
              <span className="text-slate-300 mt-6 font-bold">-</span>
              <div className="flex flex-col flex-1">
                <label className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Hora Fin</label>
                <input
                  type="time"
                  value={tempEndTime}
            ...
            <div className="flex justify-between items-center mt-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 text-sm font-bold text-white bg-slate-800 hover:bg-slate-700 rounded-xl shadow-sm transition"
                >
                  Guardar
                </button>
              </div>
